---
name: research-scout
description: Find recent alphaXiv or arXiv papers matching Zotero research interests, import strong fits, add substantive blog-style notes, and create a dated run report. Use when scouting, curating, or importing recent research papers into the user's Zotero library.
---
...
<user_prompt>
Use any focus, constraints, or limits from the user's current request.
</user_prompt>
...
Zotero app reliability:
- Zotero access depends on the local Zotero app. Before the first Zotero read or write, if Zotero calls fail because the app/API is unavailable, start Zotero with `open -a Zotero`, wait briefly, and retry.
- If Zotero was already running but local Zotero calls keep failing, restart it once with `osascript -e 'quit app "Zotero"'`, then `open -a Zotero`, wait for it to initialize, and retry the failed operation.
- Do not loop indefinitely. Try one start/restart recovery cycle, then continue conservatively if possible and mention the Zotero availability issue in the run report.
