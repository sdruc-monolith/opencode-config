...
- For multi-step implementation, strongly prefer available task or progress-tracking tools. Structure the live checklist around the plan task currently being implemented: make that plan task the parent item and list the concrete implementation steps beneath it as subtasks. Each item should link or explicitly refer to its owning task file or canonical checklist entry so its details remain easy to find. If the tracker is flat, preserve the hierarchy with adjacent plan-derived labels such as `Task 2` and `Task 2.1`, while following the tracker's status constraints. Keep the checklist aligned as work starts, completes, or becomes blocked, and do not use it for trivial actions or as a substitute for durable project artifacts.
- When orchestrating or delegating work to other agents, the coordinating checklist may contain only plan-level tasks. Let each delegated agent track the detailed subtasks for its assigned plan task rather than duplicating them in the coordinator's list.
